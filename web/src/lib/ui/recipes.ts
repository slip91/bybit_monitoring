...
export const recipes = {
  panel(options?: { tone?: "default" | "hero"; interactive?: boolean }) {
    ...
    return cn(
      "text-[var(--color-text-primary)] shadow-[0_24px_70px_rgba(0,0,0,0.32)]",
      tone === "hero"
        ? "rounded-[32px] border border-[var(--color-border)] bg-[linear-gradient(135deg,rgba(12,34,52,0.95),rgba(7,19,29,0.9))] backdrop-blur-[16px]"
        : "rounded-[28px] border border-[var(--color-border)] bg-[var(--color-surface)] backdrop-blur-[14px]",
      options?.interactive &&
        "transition duration-200 hover:-translate-y-0.5 hover:border-[rgba(84,208,200,0.3)] hover:bg-[var(--color-surface-hover)]"
    );
  },
  ...
  pill(options?: { tone?: "default" | "brand" | "success" | "running" | "stale" | "error" }) {
    ...
    return cn(
      ...
      tone === "success"  && "border-[rgba(184,233,94,0.2)] bg-[rgba(184,233,94,0.08)] text-[#d9f99d]",
  ...
  formControl() {
    return "h-11 w-full rounded-[16px] border border-[var(--color-border)] bg-[var(--color-surface-subtle)] px-4 py-3 leading-6 text-[var(--color-text-primary)] outline-none transition placeholder:text-[var(--color-text-placeholder)] focus:border-[rgba(84,208,200,0.35)] focus:bg-[rgba(255,255,255,0.05)]";
  },
  checkbox() {
  ...
  button(options?: { tone?: "primary" | "ghost" }) {
    ...
    return cn(
      ...
      tone === "primary" &&
        "border border-[var(--color-brand-border)] bg-[var(--color-brand-surface)] text-[var(--color-brand-text)] hover:border-[rgba(84,208,200,0.42)] hover:bg-[rgba(84,208,200,0.2)]",
      tone === "ghost" &&
        "border border-[var(--color-border)] bg-[var(--color-surface-subtle)] text-[var(--color-text-primary)] hover:border-[rgba(180,217,236,0.24)] hover:bg-[rgba(255,255,255,0.05)]"
    );
  },
  leverageBadge() {
    return "inline-flex items-center rounded-full border border-[rgba(180,217,236,0.16)] bg-[rgba(255,255,255,0.05)] px-2 py-0.5 text-[0.7rem] font-medium tracking-[0.08em] text-[var(--color-brand-text)]";
  },
  infoIcon() {
